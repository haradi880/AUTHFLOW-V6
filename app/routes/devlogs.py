...
def delete_comment(comment_id):
    ...
    db.session.delete(comment_row)
    devlog.comments_count = max(0, (devlog.comments_count or 0) - 1)
    db.session.commit()
    return jsonify({"status": "deleted", "count": devlog.comments_count})
...
def delete_devlog(devlog_id):
    ...
    for media in list(devlog.media):
        delete_file(media.filename, "devlogs")
    db.session.delete(devlog)
    db.session.commit()

    if _wants_json():
        return jsonify({"status": "deleted"})
    flash("DevLog deleted.", "success")
    return redirect(url_for("devlogs.index"))
